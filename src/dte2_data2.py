...
for f in emb_frame:
    ...
    for csp1, csp2 in zip(CSP1_log, CSP_emb_wtd_log):
        ...
        csp2_peaks, _ = find_peaks(csp2, height=0.2)
...
'''--------------------------------------
 埋込フレーム数の変化に伴う推定誤差結果の変化をプロット
--------------------------------------'''
fig = plt.figure(num='埋込フレーム数変化', figsize=(6, 3))
