...
for num, phase in enumerate(emb_phase):
    ...
    for k in start_frame_pos:
        ...
        # 音質検査用
        Y1zero[embedded_freq, k:k+3] = Y1zero[embedded_freq, k:k+3] * np.exp(1j*theta)
...
ax1.invert_xaxis()
ax1.plot(emb_phase, dte_log*c/1000, label='Distance Estimation Error')
ax1.set_xlim([0,180])
#ax1.set_ylim([-0.05,1.75])
ax1.set_xlabel("Embedding Phase [deg]")
ax1.set_ylabel("Estimation Distance Error[m]")

ax2 = ax1.twinx()
ax2.invert_xaxis()
ax2.plot(emb_phase, pesq_log, 'r', label='PESQ')
