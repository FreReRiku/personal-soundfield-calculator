...
for k in start_frame_pos:
    ...
    # 音質検査用
    Y1zero[embedded_freq, k:k+3] = 0.3*Y1zero[embedded_freq, k:k+3]
...
for csp1, csp2 in zip(CSP1_log, CSP_emb_wtd_log):
    ...
    csp2_peaks, _ = find_peaks(csp2, height=0.25)
    csp2_imp.append(csp2_peaks[0])
...
# 各subplotにラベルを追加
ax1.set_xlabel("Time [ms]", fontname="Arial")
...
ax1.set_ylabel("Amplitude")
#ax1.set_title('CSP1 (without embedding)')
ax1.set_xlim([1000*t[0], 1000*t[-1]/4])
ax1.set_ylim([-0.5,1.1])
plt.savefig('C1_m2.svg')

ax1.plot(1000*t, CSPs_emb)
ax1.set_xlabel("Time [ms]", fontname="Arial")
ax1.set_ylabel("Amplitude")
#ax1.set_title("CSP2 (with embedding)")
ax1.set_xlim([1000*t[0], 1000*t[-1]/4])
ax1.set_ylim([-0.5,1.1])
plt.savefig('C2_m2.svg')
'''
ax1.plot(1000*t, CSPs_emb_sub, 'lightgray', label='Difference between C1 and C2')
ax1.plot(1000*t, CSPs_emb_wtd, 'r', label='Weighted difference CSP')
ax1.set_xlabel("Time [ms]", fontname="Arial")
ax1.set_ylabel("Amplitude")
